	...
	while (game->map.matrix[++y])
	{
		...
		while (game->map.matrix[y][++x])
		{
			if (!ft_valid_char(game->map.matrix[y][x]))
				return (put_error("Invalid char in map", E_INVALID_CHAR));
			if (ft_player_pos(&game->map.matrix[y][x], y, x,
				&game->player) != 0)
				return (put_error("More than one player pos", E_PLAYER_DUP));
		}
	}
	if (game->player.pos.y == -1)
		return (put_error("No player position given", E_NO_PLAYER));
	return (0);
}
