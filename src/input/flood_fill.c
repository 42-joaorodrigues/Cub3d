...
{
	return (c == '0' || c == '1' || c == 'F' || c == 'N' || c == 'S' || c == 'E'
		|| c == 'W');
}

static int	is_surronded_by_valid(char **map, int y, int x)
{
	return (is_valid_map_char(map[y - 1][x]) && is_valid_map_char(map[y + 1][x])
		&& is_valid_map_char(map[y][x - 1]) && is_valid_map_char(map[y][x
			+ 1]));
}
...
int	ft_r_flood_fill(char **map, int y, int x)
{
	if (x < 0 || y < 0 || !map[y] || map[y][x] == '\0' || map[y][x] == '\n'
		|| map[y][x] == '1' || map[y][x] == 'F')
		return (0);
	if ((map[y][x] == '0' || map[y][x] == 'S' || map[y][x] == 'W'
			|| map[y][x] == 'E' || map[y][x] == 'N') && (y < 1 || !map[y + 1]
			|| x < 1 || (!map[y][x + 1] || map[y][x + 1] == '\n')
...
int	ft_flood_fill(char **map, int y, int x)
{
	...
	if (ft_r_flood_fill(map_dup, y, x) != 0)
		return (ft_free_matrix((void **)map_dup),
			put_error("Map is not fully surronded by walls", E_MAP_SURRONDED));
